...
const ChatConsultant: React.FC = () => {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-[100]">
      {isOpen ? (
        <div className="bg-white w-80 md:w-96 h-[500px] shadow-2xl rounded-2xl border border-black/10 flex flex-col overflow-hidden animate-in slide-in-from-bottom-5">
            ...
            {messages.map((m, i) => (
              <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[80%] p-3 rounded-2xl text-xs leading-relaxed ${
                  m.role === 'user' ? 'bg-primary text-white rounded-tr-none' : 'bg-white border border-black/5 text-zinc-700 rounded-tl-none'
                }`}>
                  {m.text}
                </div>
              </div>
          ...
          <div className="p-4 bg-white border-t border-black/5 flex gap-2">
            <input 
              ...
              className="flex-1 border-zinc-200 rounded-lg text-xs focus:ring-primary focus:border-primary"
            />
            <button 
              onClick={handleSend}
